...
\todo[inline]{figure with audio + transcription + posteriorgrams?}
...
\section{Rebalanced loss function}
...
\label{sec:discrete}
As the resulting model is sparse, we can retrieve an exact mapping by discretising the model.
...
\begin{table}
 ...
 \caption{\label{tab:abx}Within-speaker and across-speaker ABX scores (lower is better) as well as the silhouette (higher is better) for the different models for both the English and Xitsonga data sets.
